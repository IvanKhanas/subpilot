...
import com.xeno.subpilot.tgbot.ux.buttons.StartChatCallbackHandler
...
class StartChatCallbackHandlerTest {

    @MockK
    lateinit var telegramClient: TelegramClient

    private lateinit var handler: StartChatCallbackHandler

    @BeforeEach
    fun setUp() {
        handler = StartChatCallbackHandler(telegramClient)
    }

    @Test
    fun `sends help response when callback contains chat id`() {
        val callback = CallbackQuery(id = "cb-1", message = Message(chat = Chat(id = 77)))
